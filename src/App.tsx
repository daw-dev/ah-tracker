...
type AuctionListUpdatedCallback = (opportunities: Opportunity[]) => void;

const listeners: AuctionListUpdatedCallback[] = [];
...
async function auctionSearch() {
  while (true) {
    ...
    listeners.forEach((listener) => listener(opportunities));
...
function App() {
  ...
  const auctionCallback = (opportunities: Opportunity[]) => {
    console.log(opportunities.length);
    setOpportunities(opportunities.slice(0, 10));
  };

  useEffect(() => {
    listeners.push(auctionCallback);
